...
from lib.utils.image import gaussian_radius, draw_umich_gaussian, draw_msra_gaussian, gaussian_filter_density
...
class LoadImages:  # for inference
    def __init__(self, path, img_size=(1088, 608)):
        if os.path.isdir(path):
            image_format = ['.jpg', '.jpeg', '.png', '.tif', '.PNG']
...
class JointDataset(LoadImagesAndLabels):  # for training
    ...
    def __getitem__(self, files_index):
        ...
        cm = np.zeros((num_classes, output_h, output_w), dtype=np.float32)
        # cm_2 = np.zeros((num_classes, output_h, output_w), dtype=np.float32)
        cm_points = np.zeros((num_classes, output_h, output_w), dtype=np.float32)
        if self.opt.ltrb:
        ...
        for k in range(num_objs):
            ...
            if h > 0 and w > 0:
                ...
                ids[k] = label[1]
                bbox_xys[k] = bbox_xy
                cm_points[cls_id][ct_int[1], ct_int[0]] = 1

                for i in range(num_classes):
                    density = np.zeros(cm[i].shape, dtype=np.float32)
                    # print(cls_id)
                    gt = cm_points[cls_id]
